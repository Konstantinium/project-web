									...
									{if $filter.type == $filter_type && isset($filter.values)}
										{if isset($filter.slider)}
											{if $smarty.foreach.f_values.first}
												<li>
													<a href="#" rel="layered_{$filter.type}_slider" title="{l s='Cancel' mod='blocklayered'}">
										...
										{else}
											{foreach from=$filter.values key=id_value item=value}
												{if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
													<li>
														<a href="#" rel="layered_{$filter.type_lite}_{$id_value}" title="{l s='Cancel' mod='blocklayered'}">
                                                        	<i class="fa fa-times"></i>
                                                        </a>
									...
									{foreach from=$filter.values key=id_value item=value name=fe}
										...
										<li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-12">
											...
											<label for="layered_{$filter.type_lite}_{$id_value}"{if !$value.nbr} class="disabled"{else}{if isset($filter.is_color_group) && $filter.is_color_group} name="layered_{$filter.type_lite}_{$id_value}" class="layered_color" rel="{$id_value}_{$filter.id_key}"{/if}{/if}>
												...
												{else}
												<a href="{$value.link}"{if $value.rel|trim != ''} rel="{$value.rel}"{/if}>
                                                	{$value.name|escape:'html':'UTF-8'}
                                                    	{if $layered_show_qties}
                                                    		<span> ({$value.nbr})</span>
                                                        {/if}
                                                    </a>
												{/if}
								...
								{if $filter.filter_type == 0}
									...
									<div class="layered_slider_container">
										<div class="layered_slider" 
                                        	id="layered_{$filter.type}_slider" 
                                            data-type="{$filter.type}" 
                                            data-format="{$filter.format}" 
								...
								{else}
									...
									<li class="nomargin">
	                                    <div class="first-item form-group">
	                                    	<label class="form-label">{l s='From' mod='blocklayered'}({$filter.unit})</label>
	                                    	<input class="layered_{$filter.type}_range layered_input_range_min layered_input_range form-control grey" id="layered_{$filter.type}_range_min" type="text" placeholder="" value="{$filter.values[0]}"/>
	                                    </div>
	                                    <div class="form-group">
	                                    	<label class="form-label">{l s='to' mod='blocklayered'}({$filter.unit})</label> 
